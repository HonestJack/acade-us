...
static short plant_select_sem_confirm;
static bool escolha_confirmada;
...
void botao_troca() {
    ...
    plant_select_sem_confirm++;
    if(plant_select_sem_confirm >= NUM_PLANTAS)
    {
        plant_select_sem_confirm = 0;
    }
}
...
// lcd.printf("%s", lista_nomes[plant_select]);
    if (escolha_confirmada)
    {
        lcd.printf("%s \t  T_max: %.1f C", lista_nomes[plant_select], lista_temp_max[plant_select]);
    }else
    {
        lcd.printf("%s \t  Confirme", lista_nomes[plant_select_sem_confirm]);
    }
...
void confirma_troca()
{
    ...
    plant_select = plant_select_sem_confirm;
}
...
int main() {
    ...
    t3.attach(callback(&troca_celsos_firenite), 3);   
    btn_troca.fall(callback(&botao_troca));
    btn_confirma.fall(callback(&confirma_troca));
